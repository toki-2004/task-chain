...
- **APK 地址管理**：管理后台可设置「APK 端官方访问地址」并生成二维码；已安装的 APK 联网时会自动跟随切换。
- **固定入口自救**：后台可把官方地址一键推送到 Gitee 仓库配置文件（免费、国内直连），APK 配一次固定入口后，即使保存的地址彻底失联也能在下次启动时自动恢复连接。
- **手机通知**：无常驻图标——App 打开/切后台时立即检查，之后系统闹钟约 15 分钟后台检查一次，有新任务、待审核、被驳回、新反馈/申诉、申诉有回复、结束申请等待你处理的事项时**弹出系统通知**，点击直达对应任务。
- **APK 服务器分发**：最新 APK 自动存放在服务器上，任何人在浏览器打开 `http://服务器地址/apk` 即可下载（无需访问 GitHub、无需登录）；APK 内「检查更新」优先从服务器获取，国内无障碍。
- **局域网自动发现**：服务器监听 UDP 9875，APK 首次使用/连接失败时在 WiFi 内广播自动找到服务器——**同 WiFi 装上即连，零配置**。
- **救援邮箱自救**：后台配置专用邮箱后，frp 等隧道地址每次变更自动发邮件；成员 APK 自动缓存凭据，失联时读最新邮件恢复连接（POP3 零依赖实现）。免费服务商对比见 [docs/entry-providers.md](docs/entry-providers.md)。
- **全流程留痕**：任务详情可查每一个节点的受任人、发布者，以及创建/领用/反馈/申诉/提交/审核/结束/设备流转的完整时间线。

## 快速开始（局域网，3 分钟）

1. 电脑上双击 `start_server.bat`（弹出终端窗口显示地址与二维码，首次运行自动建虚拟环境装依赖；需 Python 3.10+）。停止服务双击 `stop_server.bat` 或直接关闭该终端窗口。
...
| 删除保护 | 管理员账号**不可停用、不可从后台删除**（如确需删除只能直接操作数据库）；参与过任务的用户、被任务引用/占用中的设备不可删除（只能停用/先释放），从未使用的可直接删除 |
| 管理员升降权 | 可对管理员**降权**（取消身份保留账号）与把普通用户**设为管理员**；`admin` 引导账号和当前操作者本人不可被降权 |
...
本项目基于 [PolyForm Noncommercial 1.0.0](LICENSE) 协议发布。

- ✅ 允许：个人学习、研究、测试，以及各类**非商业用途**的使用、修改与分发
- ❌ 禁止：未经作者授权的任何**商业用途**（包括但不限于出售、收费服务、集成到商业产品或盈利组织内部使用）
- 📮 商业授权：请通过 GitHub（[toki-2004](https://github.com/toki-2004)）联系作者
