...
面向"甲方 ⇄ 乙方 / 领导 ⇄ 下属"协作场景的**任务全流程系统**：发布 → 领用 → 执行 → 提交 → 审核 → 多节点接力 → 终结，全程留痕可查。
...
- **终结任务**：任意受任人可申请终结（二次确认），由**链发起人**审核；发起人本人终结直接生效。终结后整条链结束且不可恢复。
- **主页面**：所有人都有「未完成 / 待审核 / 已完成」三个列表 + 底部角标提示，距截止时间越近越前排（超期红色高亮）；「我的」= 身份信息 + 我的发布。
- **全流程留痕**：任务详情可查每一个节点的受任人、发布者，以及创建/领用/反馈/申诉/提交/审核/终结/设备流转的完整时间线。
...
| 前置设备 | 受任人须在 App 内**领用**且持有，才能提交；提交后仍持有会提醒归还；链终结**不会**自动释放设备 |
| 提交审核权 | 每个节点由**节点创建者**审核；节点 1 的创建者即链发起人 |
| 下一节点创建权 | 本节点受任人（完成者），审核通过后出现按钮 |
| 终结申请权 | 链发起人（直接生效）+ 该链任意节点的受任人（需发起人审核） |
| 数据可见性 | 链发起人、各节点创建者/受任人可见该链；管理员可见全部 |
| 账号 | 仅管理员在后台创建（用户名+密码登录），无自助注册 |
...
├── test_api.py             # API 全流程冒烟测试（72 项断言）
├── requirements.txt
├── app/
...
- 密码为 PBKDF2 哈希存储；未做 HTTPS（建议生产部署置于反向代理之后）。
